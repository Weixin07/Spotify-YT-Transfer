...
class TestMigrationIntegration:
    ...
    def test_complete_migration_with_caching(
        ...
        mock_youtube.search_video.side_effect = ["vid1", "vid2", "vid3"]
        ...
        # Verify all tracks were searched
        assert mock_youtube.search_video.call_count == 3

        # Verify all tracks were cached in database
        cached_track1 = repository.get_matched_track("Track1|Artist1")
        assert cached_track1 is not None
        assert cached_track1.youtube_id == "vid1"
        assert cached_track1.song_name == "Track1"

        cached_track2 = repository.get_matched_track("Track2|Artist2")
        assert cached_track2 is not None
        assert cached_track2.youtube_id == "vid2"

        # Reset mock to test caching
        mock_youtube.search_video.reset_mock()
        ...
        # Should NOT search YouTube (using cache)
        mock_youtube.search_video.assert_not_called()
...
class TestPlaylistIntegration:
    ...
    def test_find_missing_tracks_with_database(
        ...
        # Pre-populate database with some matched tracks
        repository.save_matched_track(
            spotify_id="Song1|Artist1",
            song_name="Song1",
            artist="Artist1",
            youtube_id="yt_vid_1",
            album="Album1",
        ...
        assert repository.get_matched_track("Song1|Artist1") is not None
        assert repository.get_matched_track("Song2|Artist2") is not None
...
class TestErrorHandlingIntegration:
    ...
    def test_failed_track_recording_and_retry(
        ...
        mock_youtube.search_video.return_value = "vid1"
        ...
        # Verify track was matched and cached
        cached = repository.get_matched_track("Track1|Artist1")
        ...
        assert all_matched[0].spotify_id == "Track1|Artist1"
