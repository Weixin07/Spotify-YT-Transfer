...
def identify_remaining_tracks():
    ...
    try:
        ...
        old_tracks_set = {
            (entry.song_name.lower().strip(), entry.artist.lower().strip()) for entry in old_entries
        }
    ...
    if playlist_matches:
        ...
        sorted_matches = sorted(playlist_matches.items(), key=lambda x: x[1]["count"], reverse=True)
